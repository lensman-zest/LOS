...
proj_list = '/path/To/proj_list.json'
...
def create_folder_and_var(var, path):
    path = os.path.expandvars(util.fix_path(path))
    hou.putenv(var, path)
...
def unset_all_scene_vars():
    ...
    variables = ['SCENE', 'SCENE_PATH', 'SAVE_PATH', 'OUTPUT_PATH', 'CACHE_PATH', 'RENDER_PATH']
...
def scene_menu(kwargs):
    ...
    scenes_path = os.path.expandvars(scenes_path)
...
def set_project(kwargs):
    ...
    project_path = os.path.expandvars(project_data['PATH'])
    hou.putenv('PROJ_PATH', project_path)
...
def set_scene(kwargs):
    ...
    # scene_name = kwargs['script_value']
    ...
    # If no project path variable
    project_path = hou.getenv('PROJ_PATH')
    if not project_path or not os.access(project_path, os.F_OK):
        util.error('Project path is invalid')
        return
    
    # If no SCENES_PATH variable
    scenes_path = hou.getenv('SCENES_PATH')
    if not scenes_path:
        util.error('Scenes path variable not found')
        return
    ...
    if not os.access(scene_path, os.F_OK):
        message = 'Scene folder "{0}" does not exists\n' \
    ...
    output = os.path.join(scene_path, 'output')
    create_folder_and_var('OUTPUT_PATH', output)
    create_folder_and_var('CACHE_PATH', os.path.join(output, 'cache'))
    create_folder_and_var('RENDER_PATH', os.path.join(output, 'render'))
...
def onCreated(kwargs):
    ...
    node.setColor(hou.Color(.58, .2, .48))
